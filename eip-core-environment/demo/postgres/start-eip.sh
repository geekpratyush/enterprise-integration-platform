#!/bin/bash
echo "======================================================================"
echo "                POSTGRESQL PLATFORM MISSION CONTROL                   "
echo "======================================================================"
...
case $CHOICE in
    1) SCENARIO="non-ssl-postgres" ;;
    2) SCENARIO="ssl-oneway-postgres" ;;
    3) SCENARIO="ssl-mtls-postgres" ;;
...
if [[ "$SCENARIO" == "ssl-"* ]]; then
    DOCKER_FILE="$EIP_SCRIPT_DIR/01_provisioning/docker-postgres-secure.yaml"
    echo "    >>> [MODE] Industrialized Secure Image Strategy"
else
    DOCKER_FILE="$EIP_SCRIPT_DIR/01_provisioning/docker-postgres.yaml"
    echo "    >>> [MODE] Standard Baseline Image Strategy"
fi

docker compose -p postgres -f "$DOCKER_FILE" down -v --remove-orphans
docker rm -f postgres-platform > /dev/null 2>&1 || true
...
docker compose -p postgres -f "$DOCKER_FILE" up -d --force-recreate

TARGET_PORT=${EIP_POSTGRES_PORT:-5432}
echo -n ">>> Waiting for PostgreSQL to stabilize (Port $TARGET_PORT)..."
until docker exec postgres-platform pg_isready -h localhost -p 5432 > /dev/null 2>&1; do
  echo -n "."
  sleep 2
done
echo -e "\n>>> POSTGRESQL is FULLY ACTIVE."
...
echo ">>> [SYSTEM] Bootstrapping EIP PostgreSQL Consumer..."
