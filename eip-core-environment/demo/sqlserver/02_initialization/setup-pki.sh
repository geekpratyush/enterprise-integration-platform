...
# 4. Packaging for SQL Server (PFX)
...
keytool -importcert -alias sqlserver-ca -file ca-cert.pem -keystore sqlserver-truststore.p12 -storetype PKCS12 -storepass changeit -noprompt
openssl pkcs12 -export -in client-cert.pem -inkey client-key.pem -out sqlserver-keystore.p12 -name "sqlserver-client" -passout pass:changeit
