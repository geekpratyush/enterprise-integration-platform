...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="v08:*">
    <xsl:element name="{local-name()}" namespace="urn:iso:std:iso:20022:tech:xsd:pacs.008.001.11">
      <xsl:apply-templates select="@*|node()"/>
    </xsl:element>
  </xsl:template>
  <xsl:template match="head:MsgDefIdr">
    <xsl:element name="MsgDefIdr" namespace="urn:iso:std:iso:20022:tech:xsd:head.001.001.02">pacs.008.001.11</xsl:element>
  </xsl:template>
</xsl:stylesheet>
